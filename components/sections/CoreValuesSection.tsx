...
const CoreValuesSection = forwardRef<HTMLDivElement>((_, ref) => {
  const sectionRef = useRef<HTMLDivElement>(null);

  // Use the core values animation hook
  useCoreValuesAnimation(sectionRef as RefObject<HTMLElement>);
  ...
  return (
    ...
    >
      ...
      {/* Decorative elements */}
      <div className="absolute top-20 right-[5%] w-64 h-64 bg-[#0066FF] opacity-[0.04] rounded-full blur-3xl"></div>
      <div className="absolute bottom-20 left-[5%] w-80 h-80 bg-[#5045E8] opacity-[0.04] rounded-full blur-3xl"></div>
      <div className="absolute top-1/2 left-1/3 w-40 h-40 bg-[#EEAE22] opacity-[0.03] rounded-full blur-2xl"></div>

      <div className="container max-w-7xl mx-auto px-6 relative z-10">
        {/* Section header */}
        <div className="text-center max-w-3xl mx-auto mb-16 md:mb-20">
          <span className="inline-block px-5 py-2 bg-blue-100 text-blue-700 text-sm font-medium rounded-full mb-5 shadow-sm section-title">
            {coreValuesData.leftContent.badge.text}
          </span>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6 section-title">
          ...
          <p className="text-lg text-gray-700 section-description">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {values.map((value, index) => (
            <div
              key={index}
              className="value-card bg-white rounded-xl overflow-hidden shadow-lg border border-gray-100 transition-all duration-300 group hover:-translate-y-2 hover:shadow-xl flex flex-col h-full"
            >
        ...
        <div className="mt-20 bg-gradient-to-r from-[#0066FF]/5 to-[#5045E8]/5 rounded-2xl p-8 md:p-10 shadow-lg border border-[#0066FF]/10 relative overflow-hidden">
          {/* Decorative elements */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-[#0066FF]/10 to-[#5045E8]/10 rounded-full -mr-20 -mt-20"></div>
          <div className="absolute bottom-0 left-0 w-48 h-48 bg-gradient-to-br from-[#0066FF]/10 to-[#5045E8]/10 rounded-full -ml-16 -mb-16"></div>

          <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
            <div className="w-full md:w-1/3 flex justify-center">
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-gradient-to-br from-[#0066FF] to-[#5045E8] flex items-center justify-center shadow-xl">
            ...
            <div className="w-full md:w-2/3">
              ...
              <div className="flex flex-wrap gap-4">
                {['Innovation', 'Collaboration', 'Leadership', 'Inclusivity'].map((tag, index) => (
                  <span key={index} className="px-4 py-2 bg-white rounded-full text-[#0066FF] font-medium shadow-sm">
                    {tag}
                  </span>
                ))}
              </div>
